...
def his_book():
    ...
    if current_user.is_admin() :
        ...
        query = ('SELECT a.*, books.title FROM (SELECT users.login, users.id, books_logs.path, books_logs.saw_at FROM users JOIN books_logs ON users.id = books_logs.user_id WHERE users.id = %s ORDER BY books_logs.saw_at DESC LIMIT %s OFFSET %s) a JOIN books ON a.path = books.id;')
        with db.connection.cursor(named_tuple = True) as cursor:
            cursor.execute(query,(current_user.id,PER_PAGES, (page-1)*PER_PAGES))
            books_list = cursor.fetchall()
        query = 'SELECT COUNT(*) AS count FROM (SELECT a.*, books.title FROM (SELECT users.login, users.id, books_logs.path, books_logs.saw_at FROM users JOIN books_logs ON users.id = books_logs.user_id WHERE users.id = %s ORDER BY books_logs.saw_at DESC ) a JOIN books ON a.path = books.id) as result'
        with db.connection.cursor(named_tuple = True) as cursor:
            cursor.execute(query,(current_user.id,))
            count = cursor.fetchone().count
